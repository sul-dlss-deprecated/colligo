...
describe 'shared/_simple_constraints.html.erb' do
  it 'should render the query and facet constraints' do
    ...
    allow(view).to receive(:render_simple_constraints_query).and_return(raw '<span>Search constraints</span>')
    allow(view).to receive(:query_has_facet_constraints?).and_return(true)
    allow(view).to receive(:render_simple_constraints_filters).and_return(raw '<span>Facet constraints</span>')
    render
